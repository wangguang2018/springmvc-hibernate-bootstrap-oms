...
function showSetting(id,sn){
    ...
    $("#changeSetting").click(function(){
        ...
        if(id<=0) {
            var checkItems = $("#pagination-form").find("input[class='checkbox-item']");
            ids = getIds(checkItems);

            if(ids == undefined || ids.length<=0){
                Dialog.danger("请选择至少一个机器");
                return;
            }
        }
        ...
        $.ajax({
            ...
            type: "POST",
            data: {"ids": ids,"machineId":id,"gameMode":gameMode,"probability":probability,"strongVoltage":strongVoltage,"smallVoltage":smallVoltage,"changeTime":changeTime,"changeWeak":changeWeak},
            dataType: "JSON",
            success: function(data) {
                if (data.code == 0) {
                    Dialog.success("设置成功", function() {
                        location.href = ctx + "/machine/set/list";
                    }, 1500);
                } else {
