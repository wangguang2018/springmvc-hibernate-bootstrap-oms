    ...
    $.ajax = function (opt) {
        ...
        var _opt = $.extend(opt, {
            error: function (jqXHR, textStatus, errorThrown) {
                ...
                try {
                    errHeader = jqXHR.errorHeader || "Error-Json";
                    errHeaderMsg = jqXHR.getResponseHeader(errHeader);
                } catch (e) {
                    errHeaderMsg = null;
                }
                ...
                if (errHeaderMsg && errHeaderMsg !== "") {
                    ...
                    var errMsg = eval("(" + errJson + ")");
                    Dialog.danger(errMsg.msg);
                }
                if (jqXHR.responseText != "") {
                    var ret = JSON.parse(jqXHR.responseText);
                    if (ret.code == -100) {
                        Dialog.danger(ret.msg);
                    }
                }
                $("button").button("reset");
                //错误统一处理方法
...
/**
 * jQuery Validate customize settings
 */
(function (window, document, $, undefined) {
    ...
    $(function () {
        ...
        if (typeof $.validator == 'undefined') {
            throw new Error('require jquery validator');
        }
        ...
        $.extend($.validator.defaults, {
            ...
            errorPlacement: function (error, element) {
                ...
                } else if (element.hasClass("checkbox") || element.hasClass("radiobox")) {
                    //element.parent().parent().append("<br>").append(error);
                } else {
            ...
            highlight: function (element, errorClass, validClass) {
                if ($(element).parents(".form-group").length > 0) {
                    //$(element).siblings('span:not(.input-group-addon)').remove();
                    //$(element).after('<span class="glyphicon glyphicon-remove form-control-feedback" aria-hidden="true"></span><span id="" class="sr-only">(error)</span>');
                    $(element).parents('.form-group').addClass('has-error').addClass('has-feedback').removeClass('has-success');
                } else {
            ...
            unhighlight: function (element, errorClass, validClass) {
                if ($(element).parents(".form-group").length > 0) {
                    //$(element).siblings('span:not(.input-group-addon)').remove();
                    //$(element).after('<span class="glyphicon glyphicon-ok form-control-feedback" aria-hidden="true"></span><span id="" class="sr-only">(success)</span>');
                    $(element).parents('.form-group').removeClass('has-error').addClass('has-feedback').addClass('has-success');
                } else {
                    if (element.type === "radio") {
        ...
        jQuery.validator.addMethod("mobile", function (value, element) {
            ...
            var mobile = /^(0|86|17951)?(13[0-9]|15[012356789]|17[012356789]|18[012356789]|14[57])[0-9]{8}$/;
            return this.optional(element) || (length == 11 && mobile.test(value));
        }, "请正确填写您的手机号码");
        // 电话号码验证
        jQuery.validator.addMethod("phoneZh", function (value, element) {
        ...
        // 匹配密码，长度在6-18之间，只能包含字符、数字、下划线和特殊字符。
        jQuery.validator.addMethod("isPassword", function (value, element) {
            return this.optional(element) || /^[a-zA-Z0-9]\S{5,31}$/.test(value);
        }, "长度在6-32之间，只能包含字符、数字、下划线和特殊字符");
        // 身份证号码验证
        ...
        jQuery.validator.addMethod("longitude", function (value, element) {
            var reg = /^(-|\+)?(180\.0{4,10}|(\d{1,2}|1[0-7]\d)\.\d{4,10})$/;
            return this.optional(element) || reg.test(value);
        }, "请输入正确的经度");
        // 纬度正则
        jQuery.validator.addMethod("latitude", function (value, element) {
            var reg = /^(-|\+)?(90\.0{4,10}|(\d|[1-8]\d)\.\d{4,10})$/;
            return this.optional(element) || reg.test(value);
        }, "请输入正确的纬度");
        ...
        function submitHandler(form, success, error) {
            var $form = $(form),
            ...
            $.ajax({
                ...
                dataType: "json",
                beforeSend: function () {
                    $button.text(text + '中...').addClass('disabled');
                },
                ...
                complete: function () {
                    $button.text(text).removeClass('disabled');
                }
            })
        }
