...
(function(root, factory) {
    ...
    var exports = {
        ...
        init: function(options) {
            ...
            exports.load(page.val(),formData);

            $(".chart-btn-search").on("click", function() {
                if($("#gtCreateTime").val() == '' && $("#ltCreateTime").val() != '')
                {
                    Dialog.warning("请填写开始日期");
                    return;
                }
                if($("#gtCreateTime").val() != '' && $("#ltCreateTime").val() == '')
                {
                    Dialog.warning("请填写结束日期");
                    return;
                }
                if($("#gtStartDate").val() == '' && $("#ltStartDate").val() != '')
                {
                    Dialog.warning("请填写起始日期");
                    return;
                }
                if($("#gtStartDate").val() != '' && $("#ltStartDate").val() == '')
                {
                ...
                var $form = $("#catchForm");
              $.ajax({
                  url: ctx + "/doll/log/chartList",
                  type: "POST",
                  data: $form.serialize(),
                  dataType: "JSON",
                  success: function(data) {
                    if(data.code == 2){
                      $("#info").html("<h1  style='margin-left:65px;margin-top: 50px'>结束日期必须大于开始日期</h1>");
                      return;
                    }
                    console.log("data:"+data);
                  /*  $("#catchChartPart").html("");
                   $("#catchChartPart").html(data); */
                  if(data.sns == undefined || data.sns==null || data.sns.length<=0){
                    $("#info").html("<h1  style='margin-left:65px;margin-top: 50px'>没有数据</h1>");
                    $("#catchChart").hide();
                    return;
                  }
                  $("#info").html("");
                  var searchType = $("select[name='searchType']").val();
                  console.log("aabb:"+searchType);
                  if(searchType == 1){//钻石
                    //getDiamond(data.prices,data.sns);
                  }else if(searchType == 2){//抓取数
                    //getcatch(data.sns,data.failRecord,data.successRecord);
                  }
              ...
              $('.chart-btn-search').trigger('click');
        },
